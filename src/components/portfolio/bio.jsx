export default function Bio() {
  return (
    <div className="row min-vh-100 mb-3 margin-top-100-phone">
      <div className="col-lg-6 d-flex flex-column justify-content-evenly">

        <div className="octagon mx-auto"></div>
        <h1 className="text-center fw-bold">
          Marcus Young<br></br>Full Stack Software Engineer
        </h1>
      </div>
      <div className="col-lg-6 d-flex flex-column justify-content-center">
        <h1 className="text-center fw-bold">Bio</h1>
        <div className="text-bg-dark rounded-4 p-3 mx-auto d-block mt-4 fs-5">
          <p className="lh-lg">
            As a Full Stack Software Engineer with extensive experience in a
            wide range of technologies including React, Node.js, MySQL, AWS, and
            more, I have the expertise to create robust and efficient web
            applications from inception to deployment. I have a proven track
            record of assembling, directing and collaborating with teams, as
            well as mentoring and educating others. My experience includes
            designing, constructing and sustaining front-end services,
            implementing responsive designs, and creating REST APIs. I'm excited
            to bring my skills and expertise to a new role where I can continue
            to grow and make an impact.
          </p>
        </div>
      </div>
    </div>
  );
}
